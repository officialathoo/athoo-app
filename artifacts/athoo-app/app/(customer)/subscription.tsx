...
export default function CustomerSubscriptionScreen() {
  ...
  async function load() {
    ...
    try {
      const [plansRes, subRes] = await Promise.all([
        api.getSubscriptionPlans("customer"),
        api.getMySubscription(),
      ]);
      const allPlans: Plan[] = (plansRes as any).plans ?? [];
      setPlans(allPlans.filter((p) => p.audience === "customer" || p.audience === "both"));
  ...
  const PERKS = [
    { icon: "star", label: "Priority booking support", color: "#F59E0B" },
    { icon: "shield", label: "Verified customer badge", color: Colors.primary },
    { icon: "percent", label: "Exclusive promo codes", color: Colors.success },
    { icon: "zap", label: "Faster response from providers", color: "#8B5CF6" },
  ];
  ...
  return (
    <View style={[styles.container, { paddingTop: topPad }]}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Athoo Premium</Text>
          <Text style={styles.headerSub}>Get more out of every booking</Text>
        </View>
      </View>
            ...
            <AnimatedCard delay={80}>
              <View style={styles.activeBanner}>
                ...
                <Pressable style={styles.cancelBtn} onPress={handleCancel} disabled={cancelling}>
                  {cancelling ? <ActivityIndicator size="small" color={Colors.error} /> : <Text style={styles.cancelBtnText}>Cancel</Text>}
                </Pressable>
              </View>
            </AnimatedCard>
          )}
            ...
            <AnimatedCard delay={80}>
              <View style={styles.pendingBanner}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.pendingSub}>Your payment is being reviewed. We'll activate it within 24 hours.</Text>
                </View>
              </View>
            </AnimatedCard>
          )}
            ...
            <View style={styles.billingToggle}>
              ...
              <Pressable onPress={() => setBilling("yearly")} style={[styles.billingBtn, billing === "yearly" && styles.billingBtnActive]}>
                <Text style={[styles.billingText, billing === "yearly" && styles.billingTextActive]}>
                  Yearly <Text style={styles.saveBadge}> · 20% off</Text>
                </Text>
              </Pressable>
            </View>
          </AnimatedCard>
          ...
          {plans.length === 0 ? (
            <AnimatedCard delay={140}>
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyTitle}>Premium plans coming soon!</Text>
                <Text style={styles.emptyText}>Stay tuned for exclusive member benefits.</Text>
              </View>
            </AnimatedCard>
          ) : (
            plans.map((plan, i) => {
          ...
          <AnimatedCard delay={380}>
            <View style={styles.noteBox}>
              ...
              <Text style={styles.noteText}>
                After subscribing, send payment via EasyPaisa / JazzCash to <Text style={{ fontWeight: "800" }}>0300-1234567</Text> and enter the transaction ID. Admin will activate within 24 hours.
              </Text>
            </View>
          </AnimatedCard>
        </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Subscribe — {selectedPlan?.name}</Text>
...
const styles = StyleSheet.create({
  ...
  heroPerk: { flexDirection: "row", alignItems: "center", gap: 10 },
  heroPerkText: { fontSize: 13, color: "#fff", fontWeight: "600" },
  ...
  cancelBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: Colors.error + "40" },
